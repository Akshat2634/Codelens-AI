...
// when more than one agent has sessions — `claude`, `codex`, and `copilot`
// views computed over just that agent's sessions. Routes select the view via
// ?source=; an unknown or absent source falls back to `all`. A bare payload
// (no `.all`) is accepted for backward compatibility and treated as `all`.
...
export function createServer(initialPayload, rebuildFn, opts = {}) {
  ...
  const pick = (req) => {
    ...
    if ((source === 'claude' || source === 'codex' || source === 'copilot' || source === 'all') && payloads[source]) {
