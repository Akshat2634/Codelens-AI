...
export function renderReportText(model) {
  ...
  L.push('');
  L.push(`  ${c.bold}${c.cyan}AI coding ROI report${c.reset} ${c.dim}· ${model.window} (${model.days} days${model.source !== 'all' ? `, ${model.source} only` : ''})${c.reset}`);
  L.push(`  ${rule}`);
...
export function renderReportMarkdown(model) {
  ...
  L.push('');
  L.push(`**${model.window}** (${model.days} days${model.source !== 'all' ? `, ${model.source} only` : ''}) · generated by [codelens-ai](https://github.com/Akshat2634/Codelens-AI)`);
  L.push('');
...
<body>
  ...
  <div class="meta">${escapeHtml(model.window)} (${model.days} days${model.source !== 'all' ? `, ${escapeHtml(model.source)} only` : ''}) · generated by codelens-ai</div>
