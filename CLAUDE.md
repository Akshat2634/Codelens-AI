...
- **Copilot pricing and context** — `copilot-parser.js` carries GitHub's published per-token rates for selectable Copilot models, including Gemini and Copilot-only models; LiteLLM is only the fallback for unknown future ids. Copilot usage comes from the final `session.shutdown` event's accumulated `modelMetrics` snapshot, while persistent `session.context_changed` events provide the current workspace and branch. A long-context model without a recorded tier is explicitly counted as estimated cost rather than silently billed at the default tier; sessions missing a shutdown usage record (crashed, ~1 in 8) are kept for correlation but marked `costZeroed` so a $0 cost can't grade as a fabricated 'A'
...
- **Repository-only project names** — Codex Desktop may record prompt-derived task folders as a session cwd even when no Git repository is involved. Those sessions keep their usage and cost but carry `projectName: null`, render as **No repository**, and are excluded from per-project aggregation. Nested workspaces and resolved moved repositories regain the actual repository name later in the pipeline.
