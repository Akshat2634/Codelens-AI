...
- **Cost mode & pricing reconciliation** (`src/cost-mode.js`) — `--cost-mode calculate|auto|display` + `--debug`. Neither Claude Code nor Codex session logs persist a real dollar-cost value historically (both are always tokens-in-log, cost-computed-by-us), so "logged cost" is redefined as the external LiteLLM overlay rate rather than a nonexistent provider-reported figure. `calculate` (default) is today's hardcoded-table behavior, unchanged. `auto` prefers the overlay rate per model, falling back to the hardcoded rate silently. `display` prefers the overlay rate but flags (`costModeIncomplete`) rather than silently substituting when a model has no overlay rate. `--debug` prints a per-model hardcoded-vs-overlay reconciliation table (`reconcilePricing`), flagging >5% divergence. The parsers compute and cache `overlayCost` per model (session-level in `modelBreakdown`, day-level in `dailyUsage[date].byModel`) alongside the existing hardcoded `.cost` — `applyCostMode` only selects which number wins, applied fresh every run (never cached) so switching modes never forces a re-parse
